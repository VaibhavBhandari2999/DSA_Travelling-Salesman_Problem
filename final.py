...
class SimAnneal(object):
    ...
    def outputs(self):
        ...
        print("\n")
        print('Best distance obtained: ', self.best_fitness)
        ...
        for i in range(len(best_route) - 1):
            for k in range(i+1, len(best_route)):
                ...
                if new_distance < best_distance:
                    ...
                    improvement = True
                    break # if improvement found, return to the top of the while loop
            if improvement:
                break
    assert len(best_route) == len(route)
#    y=run_3opt(best_route)
#    print(best_route)
    print("Total Distance : " + str(route_distance(best_route)))
    return best_route
